...
import serviceImage from "../../assets/architecture2.jpg";
...
const Architecture = () => {
  ...
  const tabData = {
    services: {
      ...
      description:
        "We provide end-to-end architectural solutions from concept to completion. Our team delivers innovative designs tailored to your specific needs and vision.",
    ...
    "why-us": {
      title: "Why Choose Our Architectural Services?",
      description:
          ...
          <div className="flex overflow-x-auto scrollbar-hide py-3">
            {tabs.map((tab) => (
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                className={`relative px-4 sm:px-6 py-2 text-sm sm:text-base font-semibold transition-all duration-300 mx-1 text-gray-600 hover:text-blue-600 hover:bg-blue-50`}
              >
                {tab.label}
                {activeTab === tab.id && (
                  <motion.div
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
              {tabData.plans.plans.slice(0, 5).map((plan, index) => (
                <motion.div
                  ...
                  className="relative group bg-white text-blue-900 rounded-2xl shadow-lg p-6 flex flex-col justify-between h-full border border-blue-200 transition-all duration-300"
                >
                  {/* Card Content */}
